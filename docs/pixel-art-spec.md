...
All Valley-mode art was generated by `scripts/gen-pixel-art.mjs` (pure Node ESM, zero
dependencies), a one-time tool that wrote the PNGs into `src/assets/valley/<company>/`.
There is no manifest.json: the company definition (`src/data/companies/<company>.ts`)
imports those PNGs with Vite `?url` and declares world/plaza/building geometry; the
renderer projects `CompanyTemplate.valley` from it (`src/map/pixel/art.ts`). Never
...
accent hex itself is not stored.
...
All PNGs are 8-bit RGBA. Sizes are pinned by the generator schema.
...
Building placement is pinned in both the generator and the company definition (single
source of truth is the `DEPTS` table in `scripts/gen-pixel-art.mjs`); background paths start at
each `door` point so they line up under every building sprite.

The interactive world remains 960×600. The company definition places the larger background at

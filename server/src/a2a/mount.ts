...
import { OPERATORS, agentCardFor } from './cards.js'
...
export function mountA2a(app: Express, deps: A2aDeps): void {
  for (const op of OPERATORS) {
    ...
    const base = `/a2a/${op.dept}`
    app.use(`${base}/.well-known/agent-card.json`, agentCardHandler({ agentCardProvider: requestHandler }))
    app.use(base, jsonRpcHandler({
      requestHandler,

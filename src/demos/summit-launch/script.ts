// Offline rehearsal dataset — the scripted launch demo runs only without a backend; every event it emits is tagged payload.simulated.
import type { AgentBlueprint, AgentDef } from '../../types'
...
import {
  SUMMIT_BUDGET_CONFIRMATION_TEMPLATE,
  SUMMIT_CLAIMS_REVIEW_TEMPLATE,
  SUMMIT_FAQ_TEMPLATE,
} from './artifacts'
...
export function heroInterviewAuto(api: EngineApi, personId: string) {
  ...
  api.onResolve(bp.id, () => heroActB(api, personId, bp.id))
...
export function heroActB(api: EngineApi, personId: string, blueprintEventId: string) {
  ...
  s.then(1200, sim(ev({
    ...
    payload: { agent: LAUNCH_AGENT_DEF, reason: blueprintEventId },
  })))
  ...
  s.then(2800, sim(ev({
    ...
    title: 'Local regex rules blocked embedded instructions',
  ...
  s.then(3400, sim(ev({
    ...
    payload: { artifact: { name: 'Claims review memo: Summit Series', type: 'Memo', template: SUMMIT_CLAIMS_REVIEW_TEMPLATE } },
  })))
  ...
  s.then(4200, sim(ev({
    ...
    payload: { artifact: { name: 'Summit Series FAQ draft', type: 'Article', template: SUMMIT_FAQ_TEMPLATE } },
  })))
...
export function heroActC(api: EngineApi, personId: string, taskId: string) {
  ...
  s.then(2600, sim(ev({
    ...
    payload: { artifact: { name: 'Q3 launch budget confirmation', type: 'Report', template: SUMMIT_BUDGET_CONFIRMATION_TEMPLATE } },
  })))
  s.then(2400, sim(ev({
    type: 'ToolCall', taskId,

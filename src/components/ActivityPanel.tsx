...
export default function ActivityPanel() {
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      {/* stats strip */}
      <div className="grid shrink-0 grid-cols-4 gap-2 border-b border-line px-3 py-2.5">
      ...
      <div className="flex shrink-0 flex-col gap-1.5 border-b border-line px-3 py-2.5">
        <div className="flex flex-wrap gap-1.5">
          <FilterChip label="All depts" active={dept === 'all'} onClick={() => setDept('all')} />
          {DEPARTMENTS.map((d) => (
      ...
      <div className="flex-1 overflow-y-auto overscroll-contain">
        {rows.length === 0 ? (
          <div className="px-3 py-10 text-center text-[11px] text-dim">No events match this filter.</div>
        ) : (
          rows.map((e) => (
            <Row
              key={e.id}
...
function Row({ event: e, highlighted, onTrace }: { event: WorldEvent; highlighted: boolean; onTrace: () => void }) {
  ...
  return (
    ...
    >
      <span className="w-[88px] shrink-0 truncate font-mono text-[10px] whitespace-nowrap text-dim tabular-nums">
        {isToday ? fmtClock(e.ts) : `${fmtDay(e.ts)} ${fmtClock(e.ts)}`}
      </span>
      <TypeChip type={e.type} />
      <span className="min-w-0 flex-1 truncate text-[12px] text-ink" title={e.detail ?? e.title}>
        {e.title}
      </span>

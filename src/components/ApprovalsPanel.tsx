...
/**
 * Tailwind emits same-property utilities alphabetically, so tint overrides lose
 * to the shared atoms' transparent border / muted text at equal specificity.
 * The `!` marks the tint as the winner; fills stay soft, rings stay hairline.
 */
    ...
    <>
      ...
      >
        ...
        <td className="px-3 py-2">
          ...
          {bp && <div className="mt-1 font-mono text-[10.5px] text-task">Blueprint review · inheritance limits</div>}
        </td>
        <td className="px-3 py-2">
          <div className="flex min-w-0 items-center gap-2">
            <span
              className={cx('flex size-7 shrink-0 items-center justify-center rounded-full border border-linebright text-[10px] font-bold', person && 'text-abyss')}
              style={{ background: person ? `hsl(${person.hue} 52% 87%)` : 'var(--color-raised)' }}
            >
              {person?.initials}
            </span>
            <div className="min-w-0">
              <div className="truncate text-[12.5px] font-medium">{person?.name ?? approval.personId}</div>
              <div className="truncate text-[10.5px] text-dim">{person?.role}</div>
            </div>
          </div>
          {isMine && <Chip className={cx('mt-1.5', TASK_TINT)}>assigned to you</Chip>}
          {viewerPerson && (
            <div className="mt-1.5 flex items-center gap-1.5 text-[10.5px] text-artifact">
        ...
        <td className="px-3 py-2 text-right">
          <div className="flex flex-wrap justify-end gap-1.5">
            {approval.kind === 'auth' ? (
              <>
                <button className="btn btn-primary h-7 px-2.5 text-[12px]" onClick={(event) => { event.stopPropagation(); onConnect() }}>
                  Connect account…
                </button>
              ...
              <>
                <button
                  className={cx('btn h-7 px-2.5 text-[12px]', approval.kind === 'blueprint' ? 'btn-primary' : 'btn-human')}
      ...
      {bp && (
        <tr className="border-b border-line bg-raised/25">
          <td colSpan={5} className="px-3 py-2">
            <div className="grid gap-3 pl-1 text-[11.5px] text-mut md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,1.3fr)]">
                ...
                <ul className="mt-1 flex flex-wrap gap-x-4 gap-y-1">
                  {bp.limits.slice(0, 3).map((l) => (
                    <li key={l} className="flex items-start gap-1.5">
                      <span className="mt-[5px] size-1 shrink-0 rounded-full bg-artifact/80" />
                      {l}
                    </li>
                  ))}
                </ul>
...
function Field({ label, value }: { label: string; value: string }) {
  return (
    <div className="mb-1.5 last:mb-0">
      ...
      <div className="text-[11.5px] leading-snug text-mut">{value}</div>
    </div>
  )
}
...
function ResolvedRow({ event }: { event: WorldEvent }) {
  ...
  return (
    <div className="flex items-center gap-2 border-b border-line/50 py-1.5 last:border-b-0">
      ...
      <span className="min-w-0 flex-1 truncate text-[11.5px] text-mut">{event.title}</span>
      {by && <span className="shrink-0 text-[11.5px] text-dim">{by}</span>}
      <span className="shrink-0 font-mono text-[10.5px] text-dim">{timeAgo(event.ts)}</span>
    </div>
  )
}
      ...
      <div className="panel anim-fadeup w-[420px] overflow-hidden">
        ...
        {mode === 'sandbox' && step === 1 && (
          <div className="p-4">
            ...
            <button
              className="mt-3 flex w-full items-center gap-3 rounded-sm border border-line bg-raised px-3 py-2.5 text-left hover:border-linebright hover:bg-hover"
              onClick={() => setStep(2)}
            >
              <span
            ...
            <div className="mt-1.5 flex w-full items-center gap-3 rounded-sm border border-line/60 px-3 py-2.5 text-[13px] text-dim">
        ...
        {mode === 'sandbox' && step === 2 && (
          <div className="p-4">
            ...
            <ul className="mt-3 flex flex-col gap-2">
              {SCOPES.map((s) => (
                <li key={s} className="flex items-start gap-2.5 rounded-sm border border-line bg-raised px-3 py-2.5">
                  <Check size={12} weight="bold" className="mt-[3px] shrink-0 text-task" />
                  <span className="text-[12.5px] leading-snug text-mut">{s}</span>
                </li>
              ))}
            </ul>
  ...
  return (
    <div className="mt-5 w-full">
      <div className="flex w-full items-center justify-center gap-1">
        <span className="flex shrink-0 items-center gap-1.5 rounded-sm border border-line bg-raised px-1.5 py-1">
          <span
            className="flex size-5 shrink-0 items-center justify-center rounded-full text-[9.5px] font-bold text-abyss"
            style={{ background: person ? `hsl(${person.hue} 52% 87%)` : 'var(--color-raised)' }}
          >
            {person?.initials}
          </span>
          <span className="text-[10.5px] font-medium text-ink">{person?.name.split(' ')[0] ?? approval.personId}</span>
        </span>

        <ScopeArrow label="grants" />

        <span className="min-w-0 shrink rounded-sm border border-human/45 bg-human/10 px-1.5 py-1">
          <span className="block truncate font-mono text-[9.5px] text-human">{scope}</span>
        </span>

        <ScopeArrow label="scoped to" />

        <span className="max-w-24 shrink-0 rounded-sm border border-line bg-raised px-1.5 py-1 text-center text-[10.5px] leading-[1.2] font-medium text-ink">
          {agentName}
        </span>
      </div>
...
function ScopeArrow({ label }: { label: string }) {
  return (
    <span className="flex shrink-0 flex-col items-center gap-px px-0.5">
      <span className="font-mono text-[9.5px] tracking-wide whitespace-nowrap text-human">{label}</span>
